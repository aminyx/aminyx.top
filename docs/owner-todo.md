...
записи стоят DNS-only (apex → 185.23.18.210 и 194.87.54.32), чтобы не ломать прямой
доступ к VPS. **DNSSEC включён на всех трёх** (10.08.2026): DNSSEC поднят на стороне
Cloudflare, DS-записи внесены у регистраторов (Spaceship для .best/.online, Namecheap
для .me). Ключ у всех зон Cloudflare общий: Key Tag 2371, Algorithm 13, Digest Type 2 —
отличается только Digest. Подпись DNS не трогает прямое соединение до VPS, сервисы
работают штатно. Проверка после публикации DS в реестрах: `AD: true`, SERVFAIL нет.

Итог по DNSSEC: **все четыре домена подписаны и валидируются**.
